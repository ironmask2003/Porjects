 ...
 */
    ...
    dim3 blockSize(1024);
    dim3 numBlocks(ceil(static_cast<float>(lines) / blockSize.x) + ceil(static_cast<float>(lines) / blockSize.x)/2);
	...
	do{
        ...
        max<<<numBlocks2, blockSize>>>(d_centroids, d_auxCentroids, d_maxDist);
        CHECK_CUDA_LAST();
        // Syncronize the device
        CHECK_CUDA_CALL( cudaDeviceSynchronize() );
